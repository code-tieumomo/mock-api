...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mt-4 flex items-center gap-2">
            ...
            }" x-ref="formReset" action="{{ route('mock-apis.archive', $mockApi) }}" method="POST">
                @csrf
                @method('PUT')
                <button x-on:click="dangerModalIsOpen = true" type="button" class="btn bg-warning text-white">Archive</button>
                <div x-cloak x-show="dangerModalIsOpen" x-transition.opacity.duration.200ms x-trap.inert.noscroll="dangerModalIsOpen" x-on:keydown.esc.window="dangerModalIsOpen = false" x-on:click.self="dangerModalIsOpen = false" class="fixed inset-0 z-30 flex items-end justify-center bg-black/20 p-4 pb-8 backdrop-blur-md sm:items-center lg:p-8" role="dialog" aria-modal="true" aria-labelledby="dangerModalTitle">
                    <!-- Modal Dialog -->
                    <div x-show="dangerModalIsOpen" x-transition:enter="transition ease-out duration-200 delay-100 motion-reduce:transition-opacity" x-transition:enter-start="opacity-0 scale-50" x-transition:enter-end="opacity-100 scale-100" class="flex max-w-lg flex-col gap-4 overflow-hidden rounded-radius border border-outline bg-surface text-on-surface dark:border-outline-dark dark:bg-surface-dark-alt dark:text-on-surface-dark">
                        <!-- Dialog Header -->
                        ...
                        <div class="px-4 text-center">
                            ...
                            <p>
                                Are you sure you want to archive the mock API? This action can be undone by restoring the mock API. After archiving, the mock API will no longer be accessible.
                            </p>
                        </div>
                        <!-- Dialog Footer -->
                        <div class="flex items-center justify-center border-outline p-4 dark:border-outline-dark">
                            <button type="button" class="w-full whitespace-nowrap rounded-radius border border-danger bg-danger px-4 py-2 text-center text-sm font-semibold tracking-wide text-on-danger transition hover:opacity-75 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-danger active:opacity-100 active:outline-offset-0" x-on:click="archiveMockApi">Archive</button>
                        </div>
                    </div>
                </div>
            </form>
            ...
            }" x-ref="formReset" action="{{ route('mock-apis.delete', $mockApi) }}" method="POST">
                @csrf
                @method('DELETE')
                <button x-on:click="dangerModalIsOpen = true" type="button" class="btn bg-danger text-white">Delete</button>
                <div x-cloak x-show="dangerModalIsOpen" x-transition.opacity.duration.200ms x-trap.inert.noscroll="dangerModalIsOpen" x-on:keydown.esc.window="dangerModalIsOpen = false" x-on:click.self="dangerModalIsOpen = false" class="fixed inset-0 z-30 flex items-end justify-center bg-black/20 p-4 pb-8 backdrop-blur-md sm:items-center lg:p-8" role="dialog" aria-modal="true" aria-labelledby="dangerModalTitle">
                    <!-- Modal Dialog -->
                    <div x-show="dangerModalIsOpen" x-transition:enter="transition ease-out duration-200 delay-100 motion-reduce:transition-opacity" x-transition:enter-start="opacity-0 scale-50" x-transition:enter-end="opacity-100 scale-100" class="flex max-w-lg flex-col gap-4 overflow-hidden rounded-radius border border-outline bg-surface text-on-surface dark:border-outline-dark dark:bg-surface-dark-alt dark:text-on-surface-dark">
                        <!-- Dialog Header -->
                        ...
                        <div class="px-4 text-center">
                            ...
                            <p>
                                Are you sure you want to delete the mock API? This action cannot be undone and will permanently delete the mock API and all data.
                            </p>
                        </div>
                        <!-- Dialog Footer -->
                        <div class="flex items-center justify-center border-outline p-4 dark:border-outline-dark">
                            <button type="button" class="w-full whitespace-nowrap rounded-radius border border-danger bg-danger px-4 py-2 text-center text-sm font-semibold tracking-wide text-on-danger transition hover:opacity-75 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-danger active:opacity-100 active:outline-offset-0" x-on:click="deleteMockApi">Delete</button>
                        </div>
                    </div>
                </div>
            </form>
